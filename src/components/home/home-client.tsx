...
interface HomeClientProps {
  classes: ClassData[]
  competitiveClasses: ClassData[]
  featuredPdfs: PdfData[]
  popularPdfs: PdfData[]
}

const competitiveExams = [
  { name: 'JEE', subtitle: 'Joint Entrance Exam', icon: Atom, bgFrom: 'from-orange-500', bgTo: 'to-amber-500', lightBg: 'bg-orange-50 dark:bg-orange-950/30', textColor: 'text-orange-600 dark:text-orange-400' },
  { name: 'NEET', subtitle: 'Medical Entrance', icon: Stethoscope, bgFrom: 'from-red-500', bgTo: 'to-rose-500', lightBg: 'bg-red-50 dark:bg-red-950/30', textColor: 'text-red-600 dark:text-red-400' },
  { name: 'CUET', subtitle: 'Central University', icon: Cuets, bgFrom: 'from-blue-500', bgTo: 'to-indigo-500', lightBg: 'bg-blue-50 dark:bg-blue-950/30', textColor: 'text-blue-600 dark:text-blue-400' },
  { name: 'REET', subtitle: 'Teacher Eligibility', icon: GraduationCap, bgFrom: 'from-purple-500', bgTo: 'to-violet-500', lightBg: 'bg-purple-50 dark:bg-purple-950/30', textColor: 'text-purple-600 dark:text-purple-400' },
  { name: 'SSC', subtitle: 'Staff Selection', icon: Landmark, bgFrom: 'from-amber-500', bgTo: 'to-yellow-500', lightBg: 'bg-amber-50 dark:bg-amber-950/30', textColor: 'text-amber-600 dark:text-amber-400' },
  { name: 'Railway', subtitle: 'Railway Recruitment', icon: Train, bgFrom: 'from-teal-500', bgTo: 'to-cyan-500', lightBg: 'bg-teal-50 dark:bg-teal-950/30', textColor: 'text-teal-600 dark:text-teal-400' },
  { name: 'Rajasthan', subtitle: 'State Exams', icon: Landmark, bgFrom: 'from-pink-500', bgTo: 'to-rose-500', lightBg: 'bg-pink-50 dark:bg-pink-950/30', textColor: 'text-pink-600 dark:text-pink-400' },
]
...
export function HomeClient({ classes, competitiveClasses, featuredPdfs, popularPdfs }: HomeClientProps) {
                      ...
                      <Link key={cls.id} href={`/class/${cls.slug}`}>
                        <div className="flex items-center gap-3 p-3 rounded-xl bg-emerald-50/50 dark:bg-emerald-950/20 hover:bg-emerald-100/80 dark:hover:bg-emerald-950/40 transition-colors group">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="text-xs text-muted-foreground">{cls._count?.subjects ?? 0} Subjects · {cls._count?.pdfs ?? 0} Notes</p>
                          </div>
                        </div>
                      </Link>
                    ))}
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-12">
        <motion.div {...fadeInUp}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
            {[
              { icon: Sparkles, title: 'Quality Content', description: 'Curated by experienced educators with years of teaching expertise', color: 'text-emerald-600 dark:text-emerald-400', bg: 'bg-emerald-50 dark:bg-emerald-950/30' },
              { icon: Zap, title: 'Instant Download', description: 'Get your notes in seconds — no waiting, no delays', color: 'text-amber-600 dark:text-amber-400', bg: 'bg-amber-50 dark:bg-amber-950/30' },
              { icon: IndianRupee, title: 'Affordable Prices', description: 'Starting from just ₹10 — quality education accessible to all', color: 'text-teal-600 dark:text-teal-400', bg: 'bg-teal-50 dark:bg-teal-950/30' },
              { icon: Shield, title: 'Secure Payments', description: 'Razorpay secured transactions — your data is always safe', color: 'text-blue-600 dark:text-blue-400', bg: 'bg-blue-50 dark:bg-blue-950/30' },
            ].map((item) => (
        ...
        <motion.div {...fadeInUp}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {testimonials.map((testimonial) => (
              <motion.div key={testimonial.name} whileHover={{ y: -4 }} transition={{ duration: 0.2 }}>
                <Card className="h-full hover:shadow-md transition-shadow">
                  <CardContent className="p-5 space-y-3">
                    <div className="flex items-center gap-1">
                      {Array.from({ length: 5 }).map((_, i) => (
                        <Star key={i} className={`w-4 h-4 ${i < testimonial.rating ? 'text-amber-400 fill-amber-400' : 'text-muted-foreground/30'}`} />
                      ))}
                    </div>
